...
// mergeError returns a merge error for a named field when we get two channel
// acceptor responses which have different values set.
func mergeError(name string, current, new interface{}) error {
...
func mergeInt64(name string, current, new int64) (int64, error) {
	...
	case current != new:
		return 0, mergeError(name, current, new)
...
func mergeDeliveryAddress(name string, current,
	...
	case !bytes.Equal(current, new):
		return nil, mergeError(name, current, new)
...
func mergeResponse(current, new ChannelAcceptResponse) (ChannelAcceptResponse,
	...
	// Merge msat values and set directly.
