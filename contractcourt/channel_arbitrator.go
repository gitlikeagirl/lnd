...
type ChannelArbitratorConfig struct {
	...
	// Blocks is a channel that receives new blocks. This channel should be
	// buffered by 1 so that the arbitrator can consume the block
	// immediately.
...
func (c *ChannelArbitrator) channelAttendant(bestHeight int32) {
	...
	for {
		...
		case blockEpoch, ok := <-c.cfg.Blocks:
			if !ok {
				return
			}
