...
func (c *anchorResolver) Resolve() (ContractResolver, error) {
	...
	case sweepRes := <-resultChan:
		...
		case nil:
			...
			spendTx = &sweepTxID
			outcome = channeldb.ResolverOutcomeAnchorRecovered

		// Anchor was swept by someone else. This is possible after the
		// 16 block csv lock.
		case sweep.ErrRemoteSpend:
			c.log.Warnf("our anchor spent by someone else")
			outcome = channeldb.ResolverOutcomeAnchorLost
		...
		case sweep.ErrTooManyAttempts:
			...
			outcome = channeldb.ResolverOutcomeAnchorUnclaimed
	...
	c.reportLock.Lock()
	if outcome == channeldb.ResolverOutcomeAnchorRecovered {
		c.currentReport.RecoveredBalance = c.currentReport.LimboBalance
	}
	c.currentReport.LimboBalance = 0
	report := c.currentReport.resolverReport(spendTx, outcome)
	c.reportLock.Unlock()
