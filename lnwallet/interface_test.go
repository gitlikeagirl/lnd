...
func sendCoins(t *testing.T, miner *rpctest.Harness,
	...
	tx, err := sender.SendOutputs(
		[]*wire.TxOut{output}, 2500, labels.UserSend,
	)
	if err != nil {
...
func testDualFundingReservationWorkflow(miner *rpctest.Harness,
	...
	err = alice.PublishTransaction(fundingTx, labels.OpenChannel)
	if err != nil {
...
func testListTransactionDetails(miner *rpctest.Harness,
	...
	burnTX, err := alice.SendOutputs(
		[]*wire.TxOut{burnOutput}, 2500, labels.UserSend,
	)
	if err != nil {
...
func newTx(t *testing.T, r *rpctest.Harness, pubKey *btcec.PublicKey,
	...
	tx, err := alice.SendOutputs(
		[]*wire.TxOut{newOutput}, 2500, labels.UserSend,
	)
	if err != nil {
...
func testPublishTransaction(r *rpctest.Harness,
	...
	err = alice.PublishTransaction(tx1, labels.UserSend)
	if err != nil {
	...
	// Publish this tx.
	err = alice.PublishTransaction(tx2, labels.UserSend)
	if err != nil {
		t.Fatalf("unable to publish: %v", err)
	}
	...
	for _, rbf := range []bool{false, true} {
		...
		err := alice.PublishTransaction(tx3, labels.UserSend)
		if err != nil {
			t.Fatalf("unable to publish: %v", err)
		}
		...
		err = alice.PublishTransaction(tx4, labels.UserSend)
		if err != nil {
			t.Fatalf("unable to publish: %v", err)
		}
		...
		err = alice.PublishTransaction(tx5, labels.UserSend)
		...
		err = alice.PublishTransaction(tx6, labels.UserSend)
		if err != expErr {
			t.Fatalf("expected ErrDoubleSpend, got: %v", err)
		}
	...
	if alice.BackEnd() != "neutrino" {
		...
		// Expect rejection.
		err = alice.PublishTransaction(tx7, labels.UserSend)
		if err != lnwallet.ErrDoubleSpend {
			t.Fatalf("expected ErrDoubleSpend, got: %v", err)
		}
	}
...
func testCreateSimpleTx(r *rpctest.Harness, w *lnwallet.LightningWallet,
	...
	for i, test := range testCases {
		...
		tx, sendErr := w.SendOutputs(outputs, feeRate, labels.UserSend)
		switch {
