...
function requiredErrCheck(){
  ...
  then
    ...
    if [ "$1" = true ]; then return 1; else return 0;fi;
  fi
}
...
waitnoerror requiredErrCheck false
echo No feature bit error for 0.11
...
waitnoerror requiredErrCheck true
echo Feature bit error for master
...
waitnoerror requiredErrCheck true
echo No feature bit error for patch
