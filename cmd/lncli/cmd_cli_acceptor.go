...
`,
	Flags: []cli.Flag{
		cli.StringFlag{
			Name:  "upfront shutdown",
			Usage: "the upfront shutdown address to use for *all requests*",
		},
	},
...
func acceptChannels(ctx *cli.Context) error {
	...
	for {
		...
		if answer == "n" {
			...
			fmt.Scanln(&acceptanceResp.Error)
		}
...
/*
	...
	// We do not set a timeout on this ctx because it needs to be long lived.
	rpcCtx, cancel := context.WithCancel(ctx)

	client, err := s.client.ChannelAcceptor(
	...
	// We make an error channel that will receive exactly one error and
	// buffer it so that the calling function consuming it does not block
	// our exit.
