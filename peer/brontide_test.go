...
func TestStaticRemoteDowngrade(t *testing.T) {
	...
	var (
		// We set the same legacy feature bits for all tests, since these are
		// not relevant to our test scenario
		rawLegacy = lnwire.NewRawFeatureVector(lnwire.AnchorsOptional)
		...
		featureOptional = lnwire.NewFeatureVector(rawFeatureOptional, nil)

		rawFeatureRequired = lnwire.NewRawFeatureVector(
			lnwire.StaticRemoteKeyRequired,
		)

		featureRequired = lnwire.NewFeatureVector(rawFeatureRequired, nil)
	)
