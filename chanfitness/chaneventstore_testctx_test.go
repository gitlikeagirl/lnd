...
type chanEventStoreTestCtx struct {
	...
	flapUpdates map[route.Vertex]uint32

	// flapCountUpdates is a channel which receives new flap counts.
	flapCountUpdates chan map[route.Vertex]uint32
...
func newChanEventStoreTestCtx(t *testing.T) *chanEventStoreTestCtx {
	testCtx := &chanEventStoreTestCtx{
		...
		flapUpdates:         make(map[route.Vertex]uint32),
		flapCountUpdates:    make(chan map[route.Vertex]uint32),
	...
	cfg := &Config{
		...
		WriteFlapCount: func(updates map[route.Vertex]uint32) error {
		...
		ReadFlapCount: func(peer route.Vertex) (uint32, error) {
...
// Quit returns the mock's quit channel, which represents the subscribe server
// indicating that it no longer wants to serve updates.
