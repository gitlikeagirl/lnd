...
func newChanEventStoreTestCtx(t *testing.T) *chanEventStoreTestCtx {
	testCtx := &chanEventStoreTestCtx{
		...
		flapUpdates:         make(peerMap),
		flapCountUpdates:    make(chan peerMap),
...
// as expected.
func (c *chanEventStoreTestCtx) assertFlapCountUpdates(expected peerMap) {
