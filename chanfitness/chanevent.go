...
type peerLog struct {
	...
	// events is a log of timestamped events observed for the peer that we
	// have committed to allocating memory to.
	onlineEvents []*event
	...
	// added and removed from this map ad they are opened and closed.
	channels map[wire.OutPoint]*channelInfo
}

// newPeerLog creates a log for a peer, taking its historical flap count as a
// parameter.
func newPeerLog(clock clock.Clock, flapCount int) *peerLog {
	return &peerLog{
		clock:     clock,
		flapCount: flapCount,
...
// event records a peer online or offline event in the log.
func (p *peerLog) onlineEvent(online bool) {
	...
	p.addOnlineEvent(online, p.clock.Now())
}

// addOnlineEvent records an online event in our event log.
func (p *peerLog) addOnlineEvent(online bool, time time.Time) {
...
// addChanel adds a channel to our log. If we have not tracked any online events
// for our peer yet, we create one with our peer's current online state so that
// we know the state that the peer had at channel start, which is required to
// calculate uptime over the channel's lifetime.
func (p *peerLog) addChanel(channelPoint wire.OutPoint) error {
	...
	// the case when we have no other channels open with the peer), and our
	// peer is currently online, we add an online event to our log so that
	// we will know the peer's start state when calculating uptime for this
	// channel.
	if len(p.onlineEvents) == 0 && p.online {
		p.addOnlineEvent(p.online, openTime)
	}
...
func (p *peerLog) getFlapCount() int {
	return p.flapCount
}
...
func (p *peerLog) getOnlinePeriods() []*onlinePeriod {
	...
	for _, event := range p.listEvents() {
